...
-(id)initWithAPIBaseURL:(NSURL *)baseUrl {
	if( nil == (self = [super init]))
		return nil;
	
	[self setBaseRequestURL:baseUrl];
	return self;
}
...
+(SMESession *)sessionWithBaseRequestURL:(NSURL  *)apiBaseUrl {
	return [[[[self class] alloc] initWithAPIBaseURL:apiBaseUrl] autorelease];
}
...
-(void)setBaseRequestURL:(NSURL *)aURL {
	if(aURL != baseRequestUrl) {
		[baseRequestUrl release];
		baseRequestUrl = [aURL retain];
	}
}
...
-(void)uploadImage:(SMEUploadRequest *)request
		  observer:(NSObject<SMEUploadObserver> *)anObserver {
	observer = anObserver;
	[self setLastUploadRequest:request];
}
